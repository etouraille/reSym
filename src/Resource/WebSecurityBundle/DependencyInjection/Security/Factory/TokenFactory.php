...
class WsseFactory implements SecurityFactoryInterface
{
    public function create(ContainerBuilder $container, $id, $config, $userProvider, $defaultEntryPoint)
    {
        ...
        $container
            ->setDefinition($providerId, new DefinitionDecorator('token.security.authentication.provider'))
            ->replaceArgument(0, new Reference($userProvider))
        ;

        $listenerId = 'security.authentication.listener.token.'.$id;
        $listener = $container->setDefinition($listenerId, new DefinitionDecorator('token.security.authentication.listener'));
